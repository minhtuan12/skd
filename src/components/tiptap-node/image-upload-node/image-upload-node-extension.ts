...
export const ImageUploadNode = Node.create<ImageUploadNodeOptions>({
    ...
    renderHTML({HTMLAttributes}) {
        return [
            "div",
            mergeAttributes({"data-type": "image-upload"}, {...HTMLAttributes, class: 'mx-auto block h-[300px]'}),
        ]
    },
                ...
                (options) =>
                    ({commands}) => {
                        return commands.insertContent({
                            type: this.name,
                            attrs: {...options, HTMLAttributes: {class: 'mx-auto block h-[300px]'}},
                        })
                    },
        }
    },
