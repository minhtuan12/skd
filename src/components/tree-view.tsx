            ...
            <AccordionPrimitive.Item value={item.id}>
                <AccordionTrigger
                    className={cn(
                        ...
                        `${activeItemId === item.id ? 'bg-gray-100' : ''}`
                    )}
                    onClick={() => {
    ...
    (
        ...
        return (
            <div
                ref={ref}
                className={cn(
                    'border select-none border-gray-300 mb-2 hover:bg-gray-100 rounded-md flex text-left items-center py-2 cursor-pointer before:right-1',
                    treeVariants(),
                    className,
                    selectedItemId === item.id && selectedTreeVariants(),
                    isDragOver && dragOverVariants(),
            ...
            >
                ...
                />
                <span className="flex-grow text-base truncate ml-5">{item.name}</span>
                <TreeActions isSelected={selectedItemId === item.id && !item.disabled}>
                    {item.actions}
                </TreeActions>
                {item.draggable ? <GripVertical className={'text-gray-400 w-5 h-5 cursor-grab'}/> : ''}
            </div>
        )
    }
)
...
>(({className, children, ...props}, ref) => (
    <AccordionPrimitive.Header className={'border border-gray-300 mb-2 rounded-md'}>
        <AccordionPrimitive.Trigger
            ref={ref}
            className={cn(
                'select-none rounded-md flex flex-1 w-full items-center py-2 transition-all first:[&[data-state=open]>svg]:first-of-type:rotate-90',
                className
            )}
            {...props}
        >
