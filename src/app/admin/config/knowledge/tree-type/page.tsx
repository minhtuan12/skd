...
export default function TreeType() {
    ...
    useEffect(() => {
        ...
        dispatch(setPageTitle('Quản lý nhóm cây'))
        setHandlers({
            submitText: 'Thêm nhóm mới',
            visibleReset: false,
            reset: () => {
            },
            submit: handleOpenCreateModal
        })
    }, [])

    return loadingFetch ? <Loader2 className={'animate-spin w-8 h-8'}/> :
        <>
            {
                        ...
                        {data?.tree_types.data.map((type: ITreeType) => (
                            <TableRow key={type._id}>
                                ...
                                <TableCell className={'flex items-center justify-center text-center gap-4'}>
                                    ...
                                    <Button className={'bg-red-500 text-white hover:bg-red-600'}><Trash/>Xóa</Button>
                                </TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
