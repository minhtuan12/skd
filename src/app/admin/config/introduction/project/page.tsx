...
export default function () {
    ...
    const {error, loading, data} = useFetchIntroduction('project');
    const {mutate, loading: loadingUpdate, isSuccess} = useUpdateIntroduction();
    const [content, setContent] = useState(data?.introduction?.project || '');
    ...
    useEffect(() => {
        if (data?.introduction?.project) {
            setContent(data?.introduction.project);
        }
    }, [data?.introduction?.project]);

    return <>
        <div className="flex items-center justify-between space-y-2 flex-wrap">
            <h2 className="text-3xl font-bold tracking-tight">Giới thiệu về dự án</h2>
            <div
