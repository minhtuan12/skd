...
export default function Form(
    {
        ...
        linkAttachedOnly: boolean
    }) {
    const {data: otherPosts, loading} = useFetchPostList('contact');
    const postSelection = useMemo(() =>
            otherPosts?.posts?.filter((item: any) => {
                return (post?._id ? (item._id !== post?._id) : item) && item.title && !item.title?.includes('http')
            }),
        [otherPosts]
    );

    return !linkAttachedOnly ? <div className={'flex gap-8 h-full pb-10'}>
        ...
        <div className={'h-full pb-10'}>
            <div className="grid gap-2">
                <Label required htmlFor="name">Link Google Form (Link nhúng)</Label>
                <Textarea
                    id="name" placeholder="Nhập link" value={post.title}
                    onChange={e => handleChangeData(e.target.value, 'title')}
                />
            </div>
        </div>
