...
export default async function KyThuatCanhTac({params}: { params: Promise<{ trang: string }> }) {
    const {trang} = await params;
    const page = parseInt(trang || '1');
    const result = await fetchKnowledge(KnowledgeTypes.farming, page);

    return <div className={'box-border flex flex-col gap-8 mt-10 lg:px-30 px-10 max-sm:px-6 pb-30'}>
        <div className={'flex flex-col gap-8 px-5'}>
            <h1 className={'font-semibold text-center text-xl'}>KỸ THUẬT CANH TÁC THÂN THIỆN VỚI SỨC KHỎE ĐẤT</h1>
                    ...
                    <PaginationContent>
                        {Array.from({length: result?.totalPages}, (_, i) => (
                            <PaginationItem key={i}>
                                <PaginationLink
                                    isActive={page === i + 1}
                                    href={`/ngan-hang-kien-thuc/ky-thuat-canh-tac/${i + 1}`}
                                >
                                    {i + 1}
                                </PaginationLink>
                            </PaginationItem>
                        ...
                        <PaginationItem>
                            <PaginationLink href={page < result.totalPages ?
                                `/ngan-hang-kien-thuc/ky-thuat-canh-tac/${page + 1}` : ''}
                            >
                                <ChevronRight/>
                            </PaginationLink>
                        </PaginationItem>
